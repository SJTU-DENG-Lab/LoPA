...
# CUDA_VISIBLE_DEVICES=0,1,2,4,5,6,7 accelerate launch --main_process_port 29510 --num_processes 7 eval.py --model dream \
#     --model_args pretrained=${model},max_new_tokens=512,diffusion_steps=512,temperature=0.2,top_p=0.95,add_bos_token=true,escape_until=true,dtype=${DTYPES_ARRAY[0]} \
#     --tasks humaneval \
#     --num_fewshot 0 \
#     --batch_size 1 \
...
for i in "${!TASKS_ARRAY[@]}"; do
    ...
    output_path=evals_results_instruct/${TASKS_ARRAY[$i]}-ns${NSHOTS_ARRAY[$i]}-len${LENGTH_ARRAY[$i]}-temp${TEMP_ARRAY[$i]}-limit${LIMITS_ARRAY[$i]}-diffsteps${LENGTH_ARRAY[$i]}-dtype${DTYPES_ARRAY[$i]}-topp09
    ...
    CUDA_VISIBLE_DEVICES=7 accelerate launch --num_processes 1 eval.py --model dream \
        --model_args pretrained=${model},max_new_tokens=${LENGTH_ARRAY[$i]},diffusion_steps=${LENGTH_ARRAY[$i]},add_bos_token=true,temperature=${TEMP_ARRAY[$i]},top_p=0.95,dtype=${DTYPES_ARRAY[$i]} \
        --tasks ${TASKS_ARRAY[$i]} \
