...
class Dream(LM):
    ...
    def _compute_selection_confidence_and_tokens(
        ...
        return selection_confidence, sampled_tokens, max_probs
    ...
    def _decode_single_sample(
    ...
    ) -> Tuple[torch.LongTensor, List[float], Optional[List[torch.LongTensor]]]:
        ...
        for step_idx in range(steps):
            ...
            selection_confidence, sampled_tokens, max_probs = self._compute_selection_confidence_and_tokens(
                remaining_logits,
                generation_config,
            )

            if collect_trace:
                step_trace.append(float(max_probs.mean().item()))
    ...
    def _save_strategy_analysis_batch(
        ...
        step_traces: Optional[List[List[float]]],
    ) -> None:
        ...
        with open(self.analysis_rank_path, "a", encoding="utf-8") as fout:
            for prompt, prompt_length, strategy_trace in zip(prompts, prompt_lengths, step_traces):
                ...
                record = {
                    ...
                    "prompt": prompt,
                    "step_mean_max_prob": strategy_trace,
                }
