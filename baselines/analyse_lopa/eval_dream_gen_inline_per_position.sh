...
limits="100"
dtypes="bfloat16"
decode_strategies="confidence_max oracle"
...
for i in "${!TASKS_ARRAY[@]}"; do
    for decode_strategy in "${DECODE_ARRAY[@]}"; do
        output_path=evals_results_instruct_perposition/${TASKS_ARRAY[$i]}-ns${NSHOTS_ARRAY[$i]}-len${LENGTH_ARRAY[$i]}-temp${TEMP_ARRAY[$i]}-limit${LIMITS_ARRAY[$i]}-diffsteps${LENGTH_ARRAY[$i]}-dtype${DTYPES_ARRAY[$i]}-topp09-${decode_strategy}-per-position
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --num_processes 8 eval_inline_diffusion_per_position.py --model dream \
