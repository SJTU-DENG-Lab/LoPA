...
# class LLaDA(LM):
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        print("request0:",requests[0].args[0])
        ds = Dataset.from_list(ds)
        gen_kwargs = requests[0].args[1]
        
        for i, batch in enumerate(ds.iter(self.batch_size)):
            contexts = batch["text"]
            # print("Self.add_bos_token:", self.add_bos_token)
            if self.add_bos_token:
                contexts = [self.tokenizer.bos_token + p for p in contexts]

            # print("before encode:",contexts)
            context_enc, attn_masks = self.tok_batch_encode(
                contexts,
                truncation=self.truncation,
            )
            # print("input_ids:",context_enc)
            # print("input_id_text:",self.tokenizer.batch_decode(context_enc, skip_special_tokens=False))
            # exit()
            out = generate(
