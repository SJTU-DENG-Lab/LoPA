...
class LLaDAEvalHarness(LM):
    ...
    def generate_until(self, requests):
        ...
        for i, req in enumerate(tqdm(requests, desc="Generating...")):
            ...
            generated_answer, nfe = stable_confident_decode(
                self.model,
                self.tokenizer,
                input_ids,
                self.gen_length,
            ...
            if self.is_instruct and 'task_id' in req.doc and str(req.doc['task_id']).lower().startswith('humaneval'):
                generated_answer = self.tokenizer.decode(actual_ids, skip_special_tokens=True)
            else:
                generated_answer = self.tokenizer.decode(actual_ids, skip_special_tokens=False)
